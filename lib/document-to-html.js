...
function DocumentConversion(options, comments) {
    ...
    function convertParagraph(element, messages, options) {
        ...
        if(element.numbering!=null) {
            var num = JSON.stringify(element.numbering.attrs).replace(/"/g, "'");
            var els = htmlPath.elements();
            for(var i = els.length-1; i>-1; i--) {
               
                var prevEl = i===0?null:els[i-1];
                var el = els[i];
                var isListItemInOL = el!=null && el.tagName==='li' &&
                    prevEl!=null && prevEl.tagName==='ol';
                var isHeading = el!=null && ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].includes(el.tagName);
                if(isListItemInOL || isHeading) {
                    Object.assign(el.attributes, {'data-numbering': num});
                    break;
                }
            }
        }
        ...
        var out = htmlPath.wrap(function() {
