...
function DocumentConversion(options, comments) {
    ...
    function convertToHtml(document) {
        ...
        .then(function() {
            ...
            var replacedDeferredHtml = replaceDeferred(html);
            var simplifiedHtml = Html.simplify(replacedDeferredHtml);
    ...
    function traverseTreeForNumbering(el, num, isInOL) {
        if(el==null) return;
        
        if(Array.isArray(el)) {
            el.forEach(function(subEl) {
                traverseTreeForNumbering(subEl, num, isInOL);
            });
        }
        
        if(!Array.isArray(el) && Array.isArray(el.children)) {
            var inOLCheck = el.tag!=null && el.tag.tagName==='ol';
            traverseTreeForNumbering(el.children, num, inOLCheck);
        }

        var nonNullTag = el!=null && el.tag!=null;
        var isListItemInOL = nonNullTag && el.tag.tagName==='li' && isInOL;
        var isHeading = nonNullTag && ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].includes(el.tag.tagName);
        if(isListItemInOL || isHeading) {
            Object.assign(el.tag.attributes, {'data-numbering': num});
        }
    }
    
    function convertParagraph(element, messages, options) {
        var out = htmlPathForParagraph(element, messages)
            .wrap(function() {
        ...
        if(element.numbering!=null || !Array.isArray(out)) {
            var num = JSON.stringify(element.numbering.attrs).replace(/"/g, "'");
            traverseTreeForNumbering(out, num, false);
        }
    ...
    function htmlPathForParagraph(element, messages) {
        var customStyle = findStyle(element);

        if (customStyle) {
            return customStyle.to.clone();
        } else {
            if (element.styleId) {
                messages.push(unrecognisedStyleWarning("paragraph", element));
            }
            var marginLeft = element.marginLeft === null ? null : (element.marginLeft / 20); //  twips to points
            var attrs = marginLeft === null ? {} : {style: "margin-left: " + marginLeft + "pt;"};
            return htmlPaths.topLevelElement("p", attrs).clone();
        }
    }
    ...
    function convertTable(element, messages, options) {
        ...
        var el = findHtmlPath(element, defaultTablePath).wrap(function() {
            return convertTableChildren(element, messages, options);
        });
        if (element.tableGrid) {
            if (!el[0].tag.attributes) {
                el[0].tag.attributes = {};
            }
            el[0].tag.attributes['data-table-grid'] = JSON.stringify(element.tableGrid);
        }
        return el;
    }
    ...
    function convertTableCell(element, messages, options) {
        ...
        var borderStyle = getBorderStyle(borders);
        if (borderStyle !== "") {
            attributes.style = (attributes.style || "") + " " + borderStyle;
        }

        var vAlign = element.verticalAlignment;
        if (vAlign) {
            attributes.style = (attributes.style || "") + " " + "vertical-align: " + vAlign + ";";
        }
    ...
    function getBorderStyle(borders) {
        if(borders == null) return "";
        return "border-width: 3px; border-style: " +
            borders.top + " " + borders.end + " " + borders.bottom + " " + borders.start + ";";
    }
    ...
    function wrapChildrenInFreshElement(
        element,
        wrapElementName,
        messages,
        options
    ...
    var elementConverters = {
        ...
        break: convertBreak,
        unrecognizedContent: function(element, messages, options) {
            return Html.freshElement('span', {'data-unrecognized-element': element.elType});
        }
    };
    return {
