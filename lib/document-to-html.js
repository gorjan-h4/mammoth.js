...
function DocumentConversion(options, comments) {
    ...
    function enrichListItemAttrs(num, li) {
        if ((li == null && li.tag == null) || li.tag.tagName !== "li") return;

        Object.assign(li.tag.attributes, {'data-numbering': num});
    }

    function traverseTreeForLI(el, num, shouldApply){
        if(el==null) return;
        
        if(Array.isArray(el)) {
            el.forEach(subEl=> traverseTreeForLI(subEl, num, shouldApply));
        }
        
        if(!Array.isArray(el) && Array.isArray(el.children)) {
            var apply = el.tag!=null && el.tag.tagName==='ol';
            traverseTreeForLI(el.children, num, apply);
        }

        if(el!=null && el.tag!=null && el.tag.tagName==='li' && shouldApply) {
            enrichListItemAttrs(num, el);
        }
    }

    function enrichNumberingAttrs(out, element) {
        if(element.numbering==null || !Array.isArray(out)) return;
        var num = JSON.stringify(element.numbering.attrs).replace(/"/g,"'");

        traverseTreeForLI(out, num, false);
    }
    ...
    function convertParagraph(element, messages, options) {
        ...
        enrichNumberingAttrs(out, element);
        return out;
    }
